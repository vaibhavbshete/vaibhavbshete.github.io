import { websiteWorkInfo, work } from "../types";
import Link from "./Link";

export default function WebsiteInfo({ work } : {work: work & {info:websiteWorkInfo}}) {
        // let enumData = [];
    // if (work.info) {
    ...
    return <div className="flex flex-wrap sm:flex-nowrap bg-gray-50 p-2" >
        ...
        }
        {
            <div className="w-full text-left px-2 flex flex-col">
                <h2 className="text-lg font-semibold px-2 py-1 border-b-2 border-b-gray-300">{work.name}</h2>
                <div className="flex flex-col justify-between flex-grow">
                    {
                        <table className="w-full mt-1">
                            <tbody>
                                <tr>
                                    <th className="px-2" scope="row">Client</th>
                                    <td>{work.info.client}</td>
                                </tr>
                                {work.info.poc && <tr>
                                    <th className="px-2" scope="row">Person of Contact</th>
                                    <td>{work.info.poc}</td>
                                </tr>}
                                {/* <Link link={work.info.link}/> */}
                                {/* {
                    ...
                    {
                        <Link link={work.info.link} className={"mx-2 w-max"} />
